...
func (mb *MetricsBuilder) AddDataPoint(resourceID, metric, aggregation, unit string, metadata []*armmonitor.MetadataValue, ts pcommon.Timestamp, val float64) {
	...
	for _, meta := range metadata {
		dp.Attributes().PutStr(*meta.Name.Value, *meta.Value)
	}
}
